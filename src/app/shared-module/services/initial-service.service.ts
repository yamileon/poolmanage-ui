...
export class InitialServiceService {
  ...
  getUserData(): Observable<Iuser[]> {
    return this.req.get<Iuser[]>(env.url+':8080/user/all');
  }

  getOneUser(username: Iuser): Observable<Iuser> {
    let urlString = env.url+':8080/user/byUsername?username=' + username.username;
    return this.req.getOne<Iuser>(urlString);

  }

  postUserData<Iuser>(user: Iuser): Observable<Iuser> {
    return this.req.post<Iuser>(env.url+':8080/user/create', user);
  }

  changeMultiple<Iuser>(username: string, user: Iuser): Observable<Iuser> {
    return this.req.put<Iuser>(env.url+':8080/user/updateUser/' + username + '/', user);
  }

  incrementWin(username: string): Observable<Iuser> {
    return this.req.putIncrement(env.url+':8080/user/updateUserInfo/win/' + username, JSON.stringify(""));
  }

  incrementLoss(username: string): Observable<Iuser> {
    return this.req.putIncrement(env.url+':8080/user/updateUserInfo/loss/' + username, JSON.stringify(""));
  }

  incrementGamesPlayed(username: string): Observable<Iuser> {
    return this.req.putIncrement(env.url+':8080/user/updateUserInfo/gamesPlayed/' + username, JSON.stringify(""));
  }

  deleteUserById<Iuser>(id: string): Observable<Iuser> {
    console.log(id);
    return this.req.delete<Iuser>(env.url+':8080/user/deleteUser?id=' + id);
  }
}
